...
int BGMPlayer::play(const std::string& path)
{
    if(_mute){
        return -1;
    }
    ...
    float rate = static_cast<float>(BGMPlayer::_volume) / 100.0f;
...
int BGMPlayer::play2d(const std::string& path)
{
    ...
    float rate = static_cast<float>(BGMPlayer::_volumeSE) / 100.0f;
...
{
    float rate = static_cast<float>(volume) / 100.0f;
